...
public class MainActivity extends AppCompatActivity implements UserListener {
    ...
    private void signOut() {
        ...
        documentReference.update(updates).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                ...
                //intent.putExtra("sender", Constants.KEY_FIRST_NAME + " " + Constants.KEY_LAST_NAME);
    ...
    public void initVideoMeeting(User user) {
        if (user.getToken() == null || user.getToken().trim().isEmpty()) {
            ...
            intent.putExtra("sender", preferenceManager.getString(Constants.KEY_FIRST_NAME) + " " + preferenceManager.getString(Constants.KEY_LAST_NAME));
            intent.putExtra("target", user.getFirstName() + " " + user.getLastName());
            startActivity(intent);
        }
    }
