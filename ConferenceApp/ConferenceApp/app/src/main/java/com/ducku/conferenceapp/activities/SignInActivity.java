        ...
        database.collection(Constants.KEY_COLLECTION_USERS)
                ...
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful() && task.getResult() != null && !task.getResult().getDocuments().isEmpty()) {
                        ...
                        preferenceManager.putString(Constants.KEY_FULL_NAME, snapshot.getString(Constants.KEY_FIRST_NAME)
                                + " " + snapshot.getString(Constants.KEY_LAST_NAME));

                        Intent intent = new Intent(getApplicationContext(), MainActivity.class);
                        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);
                        mainRepository.login(preferenceManager.getString(Constants.KEY_FULL_NAME), getApplicationContext(), () -> {
                            startActivity(intent);
                        });
                    } else {
